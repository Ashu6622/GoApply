...
export default function RegistrationQuestionnaire({ 
  ...
  const handleNext = async () => {
    if (currentStep < totalSteps) {
      ...
      try {
        ...
        if (token) {
          await api.saveProgress(currentStep + 1, profileData, token)
        }
      } catch (error) {
      ...
      try {
        const token = localStorage.getItem('token')
        if (token) {
          await api.completeProfile(profileData, token)
        }
        onComplete(profileData)
      } catch (error) {
        console.error('Failed to complete profile:', error)
        onComplete(profileData) // Fallback
      }
    }
  }
    ...
    const loadProgress = async () => {
      if (isOpen) {
        try {
          const token = localStorage.getItem('token')
          if (token) {
            const response = await api.getQuestionnaire(token)
            if (response.success && response.existingData) {
              setProfileData(response.existingData)
  ...
  // Load fields of study on dropdown open
  const loadFieldsOfStudy = async () => {
    if (fieldsOfStudy.length === 0) {
      try {
        const response = await api.getFieldsOfStudy()
        console.log('Fields API response:', response)
        if (response.success) {
          console.log('Setting fields:', response.data)
  ...
  // Load nationalities on dropdown open
  ...
  useEffect(() => {
    if (Object.keys(profileData).length > 0) {
      ...
      const token = localStorage.getItem('token')
      if (token) {
        api.saveProgress(currentStep, profileData, token).catch(console.error)
      }
    }
          ...
          <div className="space-y-6">
            ...
            <div className="text-center space-y-4">
              ...
              <Select 
                ...
                onOpenChange={(open) => open && loadFieldsOfStudy()}
              >
                <SelectTrigger className="w-full bg-background/50 backdrop-blur border-border/50">
                  <SelectValue placeholder="Select your field of study" />
                </SelectTrigger>
                <SelectContent>
                  {console.log('Rendering dropdown, fields:', fieldsOfStudy)}
          ...
          <div className="space-y-6">
            ...
            <div className="text-center space-y-4">
              ...
              <Select 
                ...
                onOpenChange={(open) => open && loadNationalities()}
              >
                <SelectTrigger className="w-full bg-background/50 backdrop-blur border-border/50">
